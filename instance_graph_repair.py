...
from src.instance_graph_new import genInstanceGraph
...
def generateLifecyleTrace(nodes):
    f = lambda eventname, index: [{'concept:name': eventname, 'time:timestamp': index,
...
class TestInstanceGraphGen(unittest.TestCase):
    def test_insertion_IG_gen(self):
        ...
        #should
        nodes = ['M3WR', 'M4RR1', 'M1WD', 'M2WD', 'M4RR2', 'M5RD', 'M4WC', 'M1TC']
        edges = [('M3WR', 'M4RR1'), ('M3WR', 'M4RR2'), ('M4RR1', 'M1WD'), ('M4RR1', 'M2WD'),
             ('M1WD', 'M5RD'), ('M2WD', 'M5RD'), ('M5RD', 'M4WC'), ('M4WC', 'M1TC')]
        nodeEventDict = {nodes[i]:trace[i]  for i in range(len(trace))}
        self.assertEqual(genInstanceGraph(traceIns, cr, orgaDict), (nodes,edges, nodeEventDict ))
    ...
    def test_deletion_IG_gen(self):
        ...
        #should
        nodes = ['M3WR', 'M4RR1', 'M1WD', 'M2WD', 'M4WC', 'M1TC']
        edges = [('M3WR', 'M4RR'), ('M4RR', 'M1WD'),
            ('M4RR1', 'M2WD'), ('M4WC', 'M1TC')]
        nodeEventDict = {nodes[i]:trace[i]  for i in range(len(trace))}
        self.assertEqual(genInstanceGraph(traceIns, cr, orgaDict), (nodes,edges, nodeEventDict ))
