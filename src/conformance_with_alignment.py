...
def loadAlignment(log: EventLog, folderPath='output/custom_cost_alignment_name_classifier/'):
    ...
    filtered_logs = (filtered_log_0, filtered_log_70, filtered_log_100,
                     filtered_log_130, filtered_log_150, filtered_log_180, filtered_log_220)
    ...
    for i in range(len(fileindexes)):
        ...
        if(len(logOfFile) != len(alignmentList)):
            ...
            alignmentLogMapping = alignmentLogMapping | mapAlignmentAndLog(
                logOfFile, alignmentList)
